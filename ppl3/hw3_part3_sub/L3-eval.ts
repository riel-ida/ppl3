...
import { L3normalEval, L3normalEvalProgram, evalNormalParse} from './L3-normal'
...
const L3applyProcedure = (proc: Value | Error, args: Array<CExp | Error>, env: Env): Value | Error =>{
    ...
    if (isPrimOp(proc)) {
        ...
        if (hasNoError(args)) return applyClosure(proc, args, env);
...
const applyClosure = (proc: Closure, args: CExp[], env: Env): Value | Error => {
    ...
    let valtovar = zip(proc.params,args);
    let res = map((x) => isLazy(first(x))? second(x) : L3applicativeEval(second(x), env), valtovar); //TODO maybe without normal eval,only x.sec
    if(!hasNoError(res)) return Error(`Bad argument: ${getErrorMessages(args)}`);
    let postEvalVartoVal = zip(proc.params, res);
    let litArgs = map((x) => (isLazy(first(x))? second(x) : valueToLitExp(second(x))), postEvalVartoVal);///
