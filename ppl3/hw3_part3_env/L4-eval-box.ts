...
import { isArray, isBoolean, isEmpty, isNumber, isString, isLazy } from "./L3-ast";
...
import { AppExp4, CompoundExp4, CExp4, DefineExp4, Exp4, IfExp4, LetrecExp4, LetExp4,
         Parsed4, ProcExp4, Program4, SetExp4, ThunkExp4 } from './L4-ast-box';
import { isAppExp4, isCExp4, isDefineExp4, isExp4, isIfExp4, isLetrecExp4, isLetExp4,
         isLitExp4, isProcExp4, isProgram4, isSetExp4, isThunkExp4 } from "./L4-ast-box";
import { parseL4, makeThunkExp4 } from "./L4-ast-box";
...
const L4applicativeEval = (exp: CExp4 | Error | ThunkExp4, env: Env): Value4 | Error => {
    ...
    isThunkExp4(exp) ? forceThunkExp4(exp) :
...
const L4applyProcedure = (proc: Value4 | Error, args: Array<CExp4 | Error>, env:Env): Value4 | Error => {
    ...
    if (isPrimOp(proc)) {
        let argVals: Array <Value4 | Error> = map((arg) => L4applicativeEval(arg, env), args);
        argVals = map((arg) => isThunkExp4(arg) ? L4applicativeEval(arg, env) : arg, argVals);
        if (hasNoError(argVals)) return applyPrimitive(proc, argVals);
        else return Error(getErrorMessages(argVals)) 
    }if (isClosure4(proc)) {
        if (hasNoError(args)) return applyClosure4(proc, args, env);
        else return Error(getErrorMessages(args)) 
    }else return Error("Bad procedure " + JSON.stringify(proc));
};

const applyClosure4 = (proc: Closure4, args: CExp4[], env:Env): Value4 | Error => {
    let vars = map((v: VarDecl) => v.var, proc.params);
    let valtoarg = zip(proc.params, args);
    let argsvalues: Array<Value4 | Error> = map((x)=> (isLazy(first(x)) ? makeThunkExp4(second(x), env) : 
                                                L4applicativeEval(second(x), env)), valtoarg);
    if(hasNoError(argsvalues)){
        return evalExps(proc.body, makeExtEnv(vars, argsvalues, proc.env));
    }
    return Error('error');
}

const forceThunkExp4 = (thunk: ThunkExp4): Value4 | Error => {
    return L4applicativeEval(thunk.arg, thunk.env);
}
...
export const applyPrimitive = (proc: PrimOp, args: Value4[]): Value4 | Error => {
    return proc.op === "+" ? (allT(isNumber, args) ? reduce((x, y) => x + y, 0, args) : Error("+ expects numbers only")) :
