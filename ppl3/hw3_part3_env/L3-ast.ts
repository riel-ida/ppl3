...
export interface VarDecl {tag: "VarDecl"; var: string; isLazy:number}; //added isLazy
...
//v:string|string[] instead of v:string
export const makeVarDecl = (v: string|string[]): VarDecl => {
    if(isArray(v)) return {tag: "VarDecl", var: v[0], isLazy:1};
    return {tag:"VarDecl", var:String(v) , isLazy:0};
}
...
//added isLazy 
export const isLazy = (x:any):boolean => {
    if(isVarDecl(x)) {
        if(x.isLazy == 1) return true;
        return false;
    }
    return false;
}
